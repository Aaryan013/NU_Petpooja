...
export const captureImage = async () => {
    try {
        const response = await axios.get(`${BASE_URL}/capture_damage`);
...
export const uploadImage = async (file) => {
    try {
        ...
        const response = await axios.post(`${BASE_URL}/upload_damage`, formData, {
...
export const detectObjects = async (imageUrl) => {
    try {
        const response = await axios.get(`${BASE_URL}/detect_damage?image_url=${encodeURIComponent(imageUrl)}`, {
